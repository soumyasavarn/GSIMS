...
if (isset($_POST["submit"])) {
    ...
    // Prepare a select statement to safely execute SQL queries with user inputs
    $query = "SELECT * FROM PRODUCTS WHERE ID = ? AND catogery = ? AND Item_name = ? AND cost = ?";
    if ($stmt = $db->prepare($query)) {
        // Bind variables to the prepared statement as parameters
        $stmt->bind_param("issi", $value1, $value2, $value3, $value4);
